...
public abstract class GameBoard extends FlowPane {
    ...
    public GameBoard(int width, int height, double buttonHeight, double buttonWidth, GameBoardLogic gameBoard, ApplicationModel model, String gameName, GridPane layout) {
        ...
        this.gameData.setGameAIStrategy(new ReversiMinimaxStrategy());
        ...
        //gameData.getGameLogic().getBoard().printBoard();
        turn = 1;
        counter = 0;
    ...
    public void drawBoard() {
        ...
        for (int row = 0; row < gameBoardHeight; row++) {
            for (int column = 0; column < gameBoardWidth; column++) {
                ...
                Button btn = tiles[id];
                btn.setOnAction(e -> {
                    GameLogic logic = gameData.getGameLogic();
                    if(gameName.equals("Tic-tac-toe")) {
                        turn = 1;
    ...
    private void setAIMove() {
        ...
        GameBoardLogic board = gameData.getGameLogic().getBoard();
    ...
    private boolean gameOver() {
        ...
        if(gameName.equals("Reversi")) {
            ...
            if(moves.size() == 0) {
                ...
                gameData.getGameLogic().getBoard().resetBoard();
                resetBoard();
            }
        }

        System.out.println(gameData.getGameLogic().gameOver());

        if(gameData.getGameLogic().gameOver() == 1) {
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("End of round");
            alert.setHeaderText(null);
            ...
            resetBoard();
            return true;
        } else if(gameData.getGameLogic().gameOver() == 2) {
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("End of round");
            alert.setHeaderText(null);
            alert.setContentText("player 2 Won!");
            ...
            resetBoard();
            return true;
        } else if(gameData.getGameLogic().gameOver() == 3) {
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("End of round");
            alert.setHeaderText(null);
    ...
    private void setMove(int pos, int state, Button btn) {
        ...
        gameData.getGameLogic().doMove(pos, state);
        counter++;
    }
